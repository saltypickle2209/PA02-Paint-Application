...
namespace LayerManager
{
    ...
    public class Layer
    {
        ...
        public void UpdateItem(List<GraphicObject> graphicObjects, Point start, Point end)
        {
            graphicObjects.ForEach(graphicObject =>
            {
                if (GraphicObjectList.Contains(graphicObject))
                {
                    ((ShapeObject)graphicObject).StartingPoint = start;

                    ((ShapeObject)graphicObject).EndingPoint = end;
